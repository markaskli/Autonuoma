...
public class AutomobilisController : Controller
{
	...
	{
		var match = AutomobilisRepo.FindAutomobolisCE(autoCE.Automobilis.VinNr);

		if(match != null) {
			ModelState.AddModelError("VIN_nr", "Field value already exists in database.");
		}
	...
	[HttpPost]
	public ActionResult Edit(string id, AutomobilisCE autoCE)
	{
		//form field validation passed?
		if (ModelState.IsValid)
		{
	...
	public void PopulateSelections(AutomobilisCE autoCE)
	{
		...
		var kuroTipai = AutomobilisRepo.ListKuroTipas();
		...
		//build select list for 'Filialai'
		{
			//initialize the destination list
			autoCE.Lists.Filialai = new List<SelectListItem>();
			...
			foreach( var mokykla in mokyklos )
			{
				...
				//build list items for the group
				foreach( var filialas in filialai ) // buvo modeliai vietoje mokyklos
				{
					var sle =
						new SelectListItem {
